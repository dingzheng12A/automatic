	...
	$("#Layer_host #add").click(function(){
		...
		if(isInteger($sshport)==false){
			...
			$.ajax({
                                type: 'post',
                                url:'/addhost',
                                data:{'host_ip':$ipaddr,'hostname':$hostname,'sshport':$sshport,'remote_user':$remote_user,'userPassword':$("#Layer_host #remote_userPassword").val(),'host_desc':$host_desc,'hostgroup':$hostgroup,'hostgroupId':$hostgroupId},
                                cache:false,
                                dataType: 'json',
								success: function(data){
									if (data.result==1){
