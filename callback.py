...
class MyCallback(CallbackModule):
    ...
    def v2_runner_on_failed(self, result,*args, **kwargs):
        ...
        if result._task.action in ('include', 'include_role'):
            ...
            if delegated_vars:
                # 自定义输出
                zdy_msg = self.zdy_stderr(json.loads(delegated_vars))
                if zdy_msg:
                    msg = "changed: [%s]%s" % (result._host.get_name(), zdy_msg)
                else:
                    msg = "changed: [%s -> %s]" % (result._host.get_name(), delegated_vars)
        ...
        # 判断是否是第一步 setup
            ...
            msg = "failure: [ %s | %s ]" % (str(result._host),str(result._host.get_groups()))
            color = C.COLOR_OK
        else:
            if delegated_vars:
                # 自定义输出
                zdy_msg = self.zdy_stdout(json.loads(delegated_vars))
                if zdy_msg:
                    msg = "failure: [%s]%s" % (result._host.get_name(), zdy_msg)
                else:
                    msg = "failure: [%s -> %s]" % (result._host.get_name(), delegated_vars)
            else:
                msg = "ok: [%s]" % result._host.get_name()
            color = C.COLOR_ERROR
    ...
    def v2_runner_on_unreachable(self, result,*args, **kwargs):
        ...
        # 判断是否是第一步 setup
            ...
            if delegated_vars:
                ...
                msg = "failure: [%s]" % result._host.get_name()
            color = C.COLOR_ERROR
