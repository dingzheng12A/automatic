...
def addhost():
	if request.method == 'POST':
		...
		host=Host(host_ip=host_ip,hostname=hostname,sshport=sshport,remote_user=remote_user,userPassword=userPassword,host_desc=host_desc,db=db)
...
@app.route('/appQuery',methods=['GET','POST'])
def appQuery():
	if 'username' in session:
		product=request.form['product']
		print "product:%s" % product
		appDeploy=AppDeploy()
		results=appDeploy.appList(product=product)
		AppList=[]
                for result in results:
                        applist={}
			applist['product']=result.product
...
def appInfo():
	if 'username' in session:
		product=request.form['product']
		print "product:%s" % product
		selectApp=request.form['selectApp']
		appName=selectApp.split('_')[0]
		version=selectApp.split('_')[1]
		appDeploy=AppDeploy()
		results=appDeploy.appInfo(product=product,appname=appName,version=version)
		AppInfo=[]
                for result in results:
                        appinfo={}
			appinfo['product']=result.product
			appinfo['appname']=result.appname
			appinfo['version']=result.version
			appinfo['package_path']=result.package_path
			appinfo['unzippath']=result.unzippath
			appinfo['runCommand']=result.command
			AppInfo.append(appinfo)
                return jsonify(AppInfo)

@app.route('/syncPacket',methods=['POST','GET'])
def syncPacket():
	if 'username' in session:
		iplist=request.form['iplist[]'].strip("[]").replace('},{','}!{')
		package_path=request.form['package_path']
		unzippath=request.form['unzippath']
		errorlist=[]
                IpList=iplist.split('!')
                for IP in IpList:
				...
				IP=json.loads(IP)
                                ...
                                ip=IP.get('ip')
                                try:
                                        appDeploy=AppDeploy(remote_host=ip)
...
@app.route('/unzipPacket',methods=['POST','GET'])
def unzipPacket():
	if 'username' in session:
		iplist=request.form['iplist[]'].strip("[]").replace('},{','}!{')
		package_path=request.form['package_path']
...
@app.route('/runDeploy',methods=['POST','GET'])
def runDeploy():
	if 'username' in session:
		iplist=request.form['iplist[]'].strip("[]").replace('},{','}!{')
		runCommand=request.form['runCommand']
...
def testConnect():
	if 'username' in session:
		...
		userPassword=request.form['userPassword']
		from User import Host
		HOST=Host(host_ip=remote_host,sshport=int(sshport),remote_user=remote_user,userPassword=userPassword)
		result=HOST.connecttest()
		return jsonify(result)
	else:
		return ''
