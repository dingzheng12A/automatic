...
class Product(object):
	...
	def addProduct(self):
		sql="select count(id) from sys_products where pname='%s'" % self.proname
		print "Running SQL: %s" % sql
		id=engine.execute(sql).first()[0]
		print "Id:%s" % id
		if id>0:
			result={'result':0}
		else:
			sql="insert into sys_products(`pname`)values('%s')" % self.proname
			try:
				engine.execute(sql)
				result={'result':1}
			except Exception,e:
	...
	def getProduct(self):
		try:
			prolist=engine.execute('select id,pname from sys_products order by id').fetchall()
                	return prolist
		except Exception,e:
			print "Has an Error:%s" % e
